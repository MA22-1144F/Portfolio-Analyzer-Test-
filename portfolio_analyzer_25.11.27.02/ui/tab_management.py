...
from PySide6.QtWidgets import (
    ...
    QSplitter, QFileDialog, QMessageBox, QProgressBar, QComboBox,
    QHeaderView, QFrame, QCheckBox, QTabWidget, QDialog, QDialogButtonBox, 
...
from data.portfolio import Portfolio, PortfolioManager, PortfolioPosition
    ...
    def run(self):
        try:
            ...
            if result:
                ...
                self.fetch_error.emit("金利データを取得できませんでした")
        except Exception as e:
...
class ManagementTab(QWidget):
    ...
    def load_optimized_portfolio(self, portfolio):
        """✨ 新規追加：最適化されたポートフォリオを読み込み
        
        Args:
            portfolio: 分析結果から作成されたPortfolioオブジェクト
        """
        try:
            # 編集タブに切り替え
            self.tab_widget.setCurrentIndex(0)
