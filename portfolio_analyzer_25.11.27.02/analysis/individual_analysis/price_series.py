...
from PySide6.QtWidgets import (
    ...
    QPushButton, QApplication, QComboBox, QGroupBox, QSpinBox,
    QFileDialog
)
...
class PriceSeriesWidget(AnalysisBaseWidget):
    ...
    def __init__(self, config=None):
        ...
        self.rows_per_page = 200  # ページ当たりの行数（調整可能）
    ...
    def get_analysis_dataframe(self) -> Optional[pd.DataFrame]:
        ...
        Returns:
            pd.DataFrame: 行=日付、列=銘柄の価格データ（共通日付のみ）
                         currencies属性に通貨情報が含まれる
        """
    ...
    def get_display_dataframe(self) -> Optional[pd.DataFrame]:
        ...
        Returns:
            pd.DataFrame: 行=日付、列=銘柄の価格データ（全日付）
                         currencies属性に通貨情報が含まれる
        """
