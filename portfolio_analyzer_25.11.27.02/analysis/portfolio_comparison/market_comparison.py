"""
market_comparison.py - ポートフォリオと市場の比較分析
証券市場線(SML)を使用して、各ポートフォリオと市場ポートフォリオを比較する
"""
...
class PortfolioMarketComparison:
    ...
    def calculate_portfolio_betas(
        ...
        各ポートフォリオのβ値とα値を計算
        Args:
            portfolio_returns: ポートフォリオ名 -> リターンのSeriesの辞書
            market_returns: 市場ポートフォリオのリターン
    ...
    def calculate_security_market_line(
        ...
        Args:
            ...
            market_expected_return: 市場期待利益率
        Returns:
            β値の範囲とSMLの期待利益率の辞書
        """
        try:
...
def create_market_comparison_plotly_chart(
    ...
    市場比較のPlotlyチャートを作成
    Args:
        portfolio_beta_results: ポートフォリオのβ値分析結果
        sml_line: 証券市場線データ
        market_expected_return: 市場期待利益率
        risk_free_rate: 無リスク利子率
        config: 設定オブジェクト
