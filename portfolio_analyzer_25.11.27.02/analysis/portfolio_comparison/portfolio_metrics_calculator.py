...
class PortfolioMetricsCalculator:
    ...
    def calculate_portfolio_metrics(
        ...
        covariance_matrix: pd.DataFrame,  # この引数は使用しない（後で再計算）
        risk_free_rate: float = 0.0
    ) -> Dict:
        """
        ポートフォリオの基本統計量を計算（修正版）
        
        Args:
            portfolio: ポートフォリオオブジェクト
            returns_data: 各資産のリターン系列 {symbol: pd.Series}
            covariance_matrix: 共分散行列（未使用、内部で再計算）
            risk_free_rate: 無リスク利子率（年率）
    ...
    def _create_common_dataframe(
        ...
        """
        共通日付でDataFrameを作成（price_series.py準拠版）
        
        日本株と米国株など、取引日が異なる資産でも共通日付を見つけて
        DataFrameを構築します。
        
        Args:
            returns_data: {symbol: pd.Series} リターンデータ
        
        Returns:
            pd.DataFrame: 共通日付のみのDataFrame
        """
        try:
            if not returns_data:
    ...
    def _build_weight_vector(
        ...
        Args:
            portfolio: ポートフォリオ
            available_symbols: 利用可能なシンボルのリスト（ソート済み）
    ...
    def calculate_returns_from_prices(
        ...
        """
        価格データからリターンを計算（日付正規化対応版）
        
        Args:
            price_data: {symbol: pd.Series} 価格データ
        
        Returns:
            Dict[str, pd.Series]: {symbol: リターン系列}
        """
        try:
            returns_data = {}
    ...
    def calculate_covariance_matrix(
        ...
        """
        共分散行列を計算（修正版 - 全資産用、実際は使用しない）
        
        Note: この関数は calculate_all_portfolios_metrics で呼ばれますが、
            実際の計算では各ポートフォリオごとに再計算するため、
            ここではダミーの行列を返します。
