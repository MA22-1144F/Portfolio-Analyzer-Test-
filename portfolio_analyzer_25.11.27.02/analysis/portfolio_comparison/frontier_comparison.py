"""
frontier_comparison.py - ポートフォリオ効率的フロンティア比較分析
複数のポートフォリオの効率的フロンティアを計算・比較する機能を提供します。
efficient_frontier.pyを参考に、以下の要素を表示：
...
class PortfolioFrontierCalculator:
    ...
    def calculate_frontier(self, price_df: pd.DataFrame, portfolio_name: str,
        ...
        Args:
            ...
            risk_free_rate: 無リスク利子率（スパン調整済み）
            steps: 計算ステップ数
        Returns:
            フロンティアデータ（volatilities, returns, weights, CAL, etc.）
        """
        try:
...
def extract_portfolio_price_data(symbols: List[str], price_data: Dict) -> Optional[pd.DataFrame]:
    ...
    Args:
        symbols: 資産シンボルのリスト
        price_data: 価格データ辞書 {symbol: pd.Series}
    Returns:
        価格データフレーム
    """
    try:
...
def create_frontier_comparison_plotly_chart(portfolios: List, price_data: Dict,
    ...
    Args:
        portfolios: ポートフォリオのリスト
        price_data: 価格データ {symbol: pd.Series}
        all_metrics: 各ポートフォリオの指標
        risk_free_rate: 無リスク利子率（スパン調整済み）
        config: 設定オブジェクト
    Returns:
        Plotly Figure
