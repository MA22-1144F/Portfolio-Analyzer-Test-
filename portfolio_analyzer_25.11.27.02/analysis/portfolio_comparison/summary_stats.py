"""
summary_stats.py - ポートフォリオ比較サマリー統計

複数のポートフォリオの主要統計量を比較テーブルとチャートで表示する機能を提供
"""
...
try:
    ...
    PlotlyFigure = object  # Fallback type for type hints
...
def create_summary_table_data(
    ...
    Args:
        metrics: ポートフォリオ別の統計量
            {portfolio_name: {'annualized_return': 0.08, ...}}
        efficiency: ポートフォリオ別の効率性指標
            {portfolio_name: {'annualized_sharpe': 0.5, ...}}
    
    Returns:
        pd.DataFrame: サマリーテーブルデータ
    """
    try:
        data = []
...
def create_summary_matplotlib_chart(
    ...
    Returns:
        matplotlib.figure.Figure: チャートfigure
    """
...
def create_summary_plotly_chart(
    ...
    Returns:
        plotly.graph_objects.Figure: インタラクティブチャート
    """
