...
class AnalysisStyles:
    ...
    def __init__(self, config=None):
        ...
        # 設定ファイルから色設定を取得
        self.COLORS = {
            "primary": config.get('ui.colors.primary', '#0078d4'),
            "primary_hover": config.get('ui.colors.primary_hover', '#106ebe'),
            "secondary": config.get('ui.colors.secondary', '#28a745'),
            "secondary_hover": config.get('ui.colors.secondary_hover', '#218838'),
            "danger": config.get('ui.colors.danger', '#dc3545'),
            "danger_hover": config.get('ui.colors.danger_hover', '#c82333'),
            "background": config.get('ui.colors.background', '#2b2b2b'),
            "surface": config.get('ui.colors.surface', '#3c3c3c'),
            "border": config.get('ui.colors.border', '#555555'),
            "text_primary": config.get('ui.colors.text_primary', '#ffffff'),
            "text_secondary": config.get('ui.colors.text_secondary', '#aaaaaa'),
            "text_accent": config.get('ui.colors.text_accent', '#0078d4'),
            "grid_line": config.get('ui.colors.grid_line', '#444444')
        }
        
        # 設定ファイルからサイズ設定を取得
        self.SIZES = {
            "progress_bar_height": config.get('ui.sizes.progress_bar_height', 18),
            "button_height": config.get('ui.sizes.button_height', 15),
            "button_font_size": config.get('ui.sizes.button_font_size', 10),
            "button_border_radius": config.get('ui.sizes.button_border_radius', 4),
        }
    ...
    def get_button_style_by_type(self, button_type: str = "primary") -> str:
        ...
        config = self.BUTTON_TYPES[button_type]
        
        return f"""
            QPushButton {{
                background-color: {config["bg_color"]};
                color: {config["text_color"]};
                border: 1px solid {config["bg_color"]};
                border-radius: {self.SIZES["button_border_radius"]}px;
                padding: 4px 8px;
                min-width: {config["width"]}px;
                max-width: {config["width"] + 50}px;
                height: {self.SIZES["button_height"]}px;
                font-size: {self.SIZES["button_font_size"]}px;
                font-weight: 500;
            }}
            QPushButton:hover {{
                background-color: {config["hover_color"]};
                border-color: {config["hover_color"]};
            }}
            QPushButton:pressed {{
                background-color: {config["bg_color"]};
                border-color: {config["bg_color"]};
            }}
    ...
    def get_table_style(self) -> str:
        return f"""
            QTableWidget {{
                ...
                background-color: {self.COLORS["background"]};
                color: {self.COLORS["text_primary"]};
                gridline-color: {self.COLORS["grid_line"]};
                selection-background-color: {self.COLORS["primary"]};
                font-size: 10px;
            }}
            QTableWidget::item {{
                padding: 2px 4px;
                border-bottom: 1px solid {self.COLORS["grid_line"]};
                border-right: 1px solid {self.COLORS["grid_line"]};
            }}
...
class AnalysisBaseWidget(QWidget):
    ...
    def create_button(self, text: str, button_type: str = "primary") -> QPushButton:
        ...
        Args:
            text: ボタンテキスト
            button_type: ボタン種類
                - "primary": 主要アクション（実行、開始、分析など）
                - "secondary": 一般的な副次アクション
                - "export": エクスポートアクション（CSV、Excel出力など）
                - "save": 保存アクション（設定保存、結果保存など）
                - "danger": 危険なアクション（リセット、削除など）
                - "info": 情報アクション（詳細、ヘルプなど）
                - "small": 小さなアクション（設定、オプションなど）
                - "neutral": 中性的なアクション（キャンセル、閉じるなど）
        """
