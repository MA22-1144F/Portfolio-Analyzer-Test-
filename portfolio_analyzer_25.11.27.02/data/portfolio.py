...
class Portfolio:
    ...
    def validate(self) -> List[str]:
        ...
        if not self.positions:
            errors.append("資産が選択されていません")
        
        for pos in self.positions:
            if pos.weight < 0:
                errors.append(f"{pos.asset.symbol}: ウエイトが負の値です")
            if pos.weight > 10:  # 1000%のリミット
                errors.append(f"{pos.asset.symbol}: ウエイトが過大です（最大1000%）")
...
class PortfolioManager:
    ...
    def list_portfolios(self) -> List[Tuple[str, str, str]]:
        """保存されたポートフォリオのリストを取得
        Returns: [(file_name, portfolio_name, created_at_display), ...]
        """
        portfolios = []
    ...
    def save_portfolio(self, portfolio: Portfolio, file_name: Optional[str] = None) -> str:
            ...
            Args:
                portfolio: 保存するポートフォリオ
                file_name: 既存のファイル名（省略時は新規作成）
